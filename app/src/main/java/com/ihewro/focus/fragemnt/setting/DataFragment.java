...
public class DataFragment extends SettingFragment{
    ...
    public void initListener() {
        ...
        fix_database.setOnPreferenceClickListener(preference -> {
            new MaterialDialog.Builder(getActivity())
                    .title("确定修复数据库？")
                    .content("修复数据库可以删除错误数据。如果您是从旧版本升级到2.0+版本，使用该功能可以将您的收藏数据恢复。")
                    .positiveText("修复")
                    .negativeText("取消")
                    .onPositive((dialog, which) -> new FixDataTask(getActivity()).execute())
                    .show();

            return false;
        });

        back_up.setOnPreferenceClickListener(preference -> {
            new MaterialDialog.Builder(getActivity())
                    .title("为什么需要备份？")
        ...
        recover_data.setOnPreferenceClickListener(preference -> {
            new RecoverDataTask(getActivity()).execute();

            return false;
        });

        clean_data.setOnPreferenceClickListener(preference -> {
            //显示输入弹窗
                    ...
                    .input("", "", (dialog, input) -> {
                        ...
                        if (pattern.matcher(num).matches() && !StringUtil.trim(num).equals("")) {
                            ...
                            new Thread(() -> {
                                ...
                                UIUtil.runOnUiThread(getActivity(), () -> {
